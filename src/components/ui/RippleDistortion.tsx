...
const RippleDistortion = ({
  ...
  useEffect(() => {
    ...
    if (videoElement) {
      ...
      imageTexture.image = videoElement;
      
      const onVideoReady = () => {
        if (disposed) return;
        compositeUniforms.uTextureSize.value = [videoElement.videoWidth || 1, videoElement.videoHeight || 1];
    ...
    const loop = (now: number) => {
      ...
      // Update texture on every frame if video is playing
